...
icon = Image.open('Streamlit/logo.png')
banner = Image.open('Streamlit/homepage.jpg')
sidebar = Image.open('Streamlit/DSA_logo.png')

st.set_page_config(page_title = 'Case Study', page_icon = icon, layout = 'wide')
st.sidebar.image(sidebar)
st.image(banner)
st.title("Case Study Deployment")
st.text("Loan Prediction and Water Portability Prediction")
page = st.sidebar.selectbox("SELECT", ('Homepage', 'Data preperation', 'Prediction'))

if page == 'Homepage':
    data_choice = st.selectbox("Select data", ("Water Portability", "Loan Prediction"))
    if data_choice == "Water Portability":
        df = pd.read_csv('Streamlit/water_potability.csv')
    else:
        df = pd.read_csv('Streamlit/loan_pred.csv')
    st.markdown("Data")
    st.dataframe(df)
    ...
    if(len(df) != len(df.drop_duplicates())):
        st.markdown("There is no duplicate")
    else:
        st.markdown("There are duplicates")

    st.markdown("Balance of values of label")
    ...
    for i, columns in enumerate(filtered_numerical_columns):
    ...
    tts = st.text_input(label = 'Enter the size of test data. For example 0.2', value = 0.2)
    random_state = st.text_input(label = 'Enter the random state. For example 42', value = 42)
    ...
    if st.button('Build model') == True:
        ...
        if len(categorical_columns) > 0:
            if cat_encoding == 'OneHotEncoder':
                ...
                X_train = pd.concat([X_train.drop(categorical_columns, axis=1).reset_index().drop('index', axis=1),
                                     pd.DataFrame(X_train_encoded)], axis=1)
                X_test = pd.concat([X_test.drop(categorical_columns, axis=1).reset_index().drop('index', axis=1),
